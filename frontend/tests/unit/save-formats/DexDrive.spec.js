...
describe('DexDrive PS1 save format', () => {
  ...
  it('should convert a single save file to a raw file', async () => {
    ...
    for (let i = 0; i < dexDriveSaveData.getSaveFiles().length; i += 1) {
      ...
      console.log(`Found block ${saveFile.block} contains file '${saveFile.filename}' with comment '${saveFile.comment}' and raw save data length ${saveFile.rawData.byteLength}`);
