...
const MEGA_SD_NEW_EEPROM_FILENAME = `${DIR}/Knuckles' Chaotix (Japan, USA) (En).SRM`;
const MEGA_SD_RAW_NEW_EEPROM_FILENAME = `${DIR}/Knuckles' Chaotix (Japan, USA) (En)-raw.sav`;

/*
const MEGA_SD_NEW_EEPROM_2_FILENAME = `${DIR}/Wonder Boy in Monster World (USA, Europe)-new-style.SRM`;
const MEGA_SD_RAW_NEW_EEPROM_2_FILENAME = `${DIR}/Wonder Boy in Monster World (USA, Europe)-new-style-raw.sav`;
*/
...
describe('Flash cart - Genesis - Mega SD - 32X', () => {
  it('should convert a Mega SD SRAM save in the new style to raw format', async () => {
    ...
    ArrayBufferUtil.writeArrayBuffer(MEGA_SD_RAW_NEW_SRAM_FILENAME, flashCartSaveData.getRawArrayBuffer());
  });
  ...
  */
  ...
  it('should convert a Mega SD EEPROM save in the new style to raw format', async () => {
    ...
    ArrayBufferUtil.writeArrayBuffer(MEGA_SD_RAW_NEW_EEPROM_FILENAME, flashCartSaveData.getRawArrayBuffer());
  });
  ...
  */
  ...
  /*
  it('should convert a second Mega SD EEPROM save in the new style to raw format', async () => {
    const flashCartArrayBuffer = await ArrayBufferUtil.readArrayBuffer(MEGA_SD_NEW_EEPROM_FILENAME);
    const rawArrayBuffer = await ArrayBufferUtil.readArrayBuffer(MEGA_SD_RAW_NEW_EEPROM_FILENAME);
  ...
  it('should convert a second raw EEPROM save to the new style', async () => {
    const flashCartArrayBuffer = await ArrayBufferUtil.readArrayBuffer(MEGA_SD_NEW_EEPROM_FILENAME);
    const rawArrayBuffer = await ArrayBufferUtil.readArrayBuffer(MEGA_SD_RAW_NEW_EEPROM_FILENAME);
